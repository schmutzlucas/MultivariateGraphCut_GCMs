...
GraphCutHellinger2D_stoch_par <- function(
  ...
  results <- foreach(k = 1:N_IT, .packages = c("gcoWrapR", "RcppXPtrUtils")) %dopar% {
    ...
    tryCatch({
      ...
      list("label_attribution" = label_attribution, "Data and smooth cost" = data_smooth_list, 'h_dist' = h_dist)
    }, error = function(e) {
      # Optional: return the error to handle it after the loop
      list(error = paste("Error in iteration", k, ":", e$message))
    })
