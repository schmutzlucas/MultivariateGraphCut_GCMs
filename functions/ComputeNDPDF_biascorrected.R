# Set up parallel processing option (16 GiB maximum globals)
...
process_model_pdf <- function(m_idx, model_names, nlon, nlat, variables, data_dir,
  ...
  cat("Processing model:", as.character(model_names[[m_idx]]), "\n")


  n_vars <- length(variables)
  # Lists to hold bias-corrected data (for present and future)
  corrected_data_present_list <- vector("list", n_vars)
  corrected_data_future_list  <- vector("list", n_vars)
  # Arrays to record out-of-range counts [nlon, nlat, n_vars]
  ...
  for (v in seq_along(variables)) {
    ...
    # Create arrays for corrected data for this variable.
  ...
  for (i in seq_len(nlon)) {
    for (j in seq_len(nlat)) {
      ...
      # Count out-of-range values for present:
