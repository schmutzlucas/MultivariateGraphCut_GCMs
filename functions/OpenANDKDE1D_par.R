OpenAndKDE1D_par <- function (model_names, variables,
  ...
  for(var in variables){
    ...
    for(model_name in model_names){
      ...
      if (length(file_path) == 1) {
        ...
        # Read data from NetCDF file for each grid point
        data_list <- list()
        ...
        foreach(i = iter(seq_along(lon)), .combine = 'cbind',
                .packages = c("ncdf4"), .export = c("data_list", "min", "iyyyy", "nbins1d", "m", "var")) %dopar% {
