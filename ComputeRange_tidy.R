...
# 7. Merges the computed ranges into a final global range.
...
# Load additional libraries not in the list
library(future)
library(future.apply)
library(ncdf4)
library(devtools)
install_github("thaos/gcoWrapR")  # Custom Graph Cut Optimization package
...
# --------- 5. Function to Calculate Min and Max Ranges ---------
...
# --------- 7. Assign Results to Individual Variables ---------
range_pr <- range_results[[1]]
range_tas <- range_results[[2]]
range_psl <- range_results[[3]]

# --------- 8. Save Ranges for Each Variable Separately ---------
saveRDS(range_pr, 'ranges/pr_log_range_AllModelsPar_1950-2022_70deg.rds')
saveRDS(range_tas, 'ranges/tas_range_AllModelsPar_1950-2022_70deg.rds')
saveRDS(range_psl, 'ranges/psl_range_AllModelsPar_1950-2022_70deg.rds')
...
for (i in seq_along(lon)) {
  for (j in seq_along(lat)) {
    final_ranges[[1]][i, j, ] <- range(range_pr[i, j, , ], na.rm = TRUE)
    final_ranges[[2]][i, j, ] <- range(range_tas[i, j, , ], na.rm = TRUE)
    final_ranges[[3]][i, j, ] <- range(range_psl[i, j, , ], na.rm = TRUE)
  }
}
