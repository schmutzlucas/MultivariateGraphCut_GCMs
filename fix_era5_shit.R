...
GC_result_hellinger_test_new_era5 <- list()
GC_result_hellinger_test_new_era5 <- tryCatch({
...
for(var in variables){
  for(l in 0:(length(model_names))){
    islabel <- which(GC_result_hellinger_test_new_era5$label_attribution == l)
...
h_dist_map <- array(NA, dim = dim(GC_result_hellinger_test_new_era5$label_attribution))
...
# Adjust your data to map to the -180 to 180 range for longitude
test_df$lon <- ifelse(test_df$lon > 180, test_df$lon - 360, test_df$lon)

# Plotting with Mollweide projection
p5 <- ggplot() +
  ...
  scale_fill_gradient(low = "white", high = "#015a8c", limits = c(0.1, 0.7), oob = scales::squish) +
  ...
  labs(fill = 'Hellinger \nDistance') +
  theme_bw() +
  theme(legend.key.size = unit(1, 'cm'), legend.key.height = unit(1.4, 'cm'), legend.key.width = unit(0.4, 'cm'),
        legend.title = element_text(size = 16), legend.text = element_text(size = 12),
        plot.title = element_text(size = 24), plot.subtitle = element_text(size = 20, hjust = 0.5),
        axis.text = element_text(size = 14), axis.title = element_text(size = 16)) +
  coord_map("mollweide") # Using the Mollweide
p5
...
name <- paste0('figure/H_dist_future_GC_hellinger_new_26models_present')
ggsave(paste0(name, '.pdf'), plot = p5, width = 35, height = 25, units = "cm", dpi = 300)
ggsave(paste0(name, '.png'), plot = p5, width = 35, height = 25, units = "cm", dpi = 300)
