...
# Reshape the results into the original pdf_values format
pdf_values <- array(results, dim = c(dim(tas)[1], dim(tas)[2], nbins^2))
...
for (i in 1:100) {
  for (j in 1:104) {
    ...
    dump <- kde2d(times_series[, 1], times_series[, 2],
                    n = nbins, h = 1, lims = c(range_tas, range_pr))
